...
async def login(sid, data):
    ...
    if user_id in connected_admins.values(): #search again for the user id only this time in the user list
     ...
     if any(admin['id'] == user_id and admin['status'] == 'active'for admin in admins): #search for the user id in allowed users list // check if the username is allowed
     ...
     elif any(user['id'] == user_id and user['status'] == 'active'for user in users) or (user_id =='benchmarker'): #search for the user id in allowed users list // check if the username is allowed
        print(f"✅ Login success for {user_id}") #if it is mark connecton
...
async def locator(sid, data):
    ...
    INSERT INTO requests (
        ...
        people, task, roomnumber, priority, overrided_by
    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
""", (
    RID, user_id, None, trimmed, None, None, 
    counter, task, room_number, dropdown, None
))
    nostosdb.commit()
...
async def delete_request_message(sid,data):
    RID = data.get("RID")
    values = (RID,)
    nostoscur.execute(delete_request_from_chat, values)
    nostosdb.commit()
    await sio.emit("refresh_messages")
...
async def delete_message(sid,data):
    id = data.get("id")
    values = (id,)
    nostoscur.execute(delete_message_from_chat, values)
    nostosdb.commit()
    await sio.emit("refresh_messages")
...
async def edit_message(sid,data):
    ...
    nostoscur.execute(edit_message_from_chat, values)
    nostosdb.commit()
    await sio.emit("refresh_messages")
...
async def admin_password(sid,data):
    ...
    values = (admin_id,)
    nostoscur.execute(sql_find_admin_password,values)
    row = nostoscur.fetchone()
    if row is None:
        await sio.emit("Admin_Password_Result", {
            "success": False,
        }, to=sid)
        return
...
async def return_rooms(user_id,room_to_find):
    ...
    values = (room_to_find,)
    nostoscur.execute(sql_return_priority_from_room,values)
    row_ = nostoscur.fetchone()
    if row_ is not None:
     level = row_[0]
    else:
     level= None 
    ...
    #level = priority
      ...
      #secondary_list = await propose_room(room_to_find)
...
async def work_sender():
    nostoscur.execute("SELECT name, userid, active FROM users")
    users_ = [{'name': row[0], 'id': row[1], 'status': 'active' if row[2] else 'inactive'} for row in nostoscur.fetchall()]

    

    nostoscur.execute("SELECT name, groomid, active FROM grooms")
    grooms_ = [{'name': row[0], 'id': row[1], 'status': 'active' if row[2] else 'inactive'} for row in nostoscur.fetchall()]
...
async def add_user(sid,data):
    ...
    nostoscur.execute(
    sql.SQL("""
        INSERT INTO {} (new_user, name, active)
        VALUES (%s, %s, %s)
