...
sid_limits = defaultdict(list)
ip_limits = defaultdict(list)
login_attempts = defaultdict(list)

def get_ip(environ):
    # Handles reverse proxy forwarding
    return (
        environ.get("HTTP_X_FORWARDED_FOR", "").split(",")[0].strip()
        or environ.get("REMOTE_ADDR", "unknown")
    )

def is_ip_limited(ip, limit=10, window=10):
    now = time.time()
    ip_limits[ip] = [t for t in ip_limits[ip] if now - t < window]
...
def is_sid_limited(sid, limit=3, window=5):
    now = time.time()
    sid_limits[sid] = [t for t in sid_limits[sid] if now - t < window]
    
    if len(sid_limits[sid]) >= limit:
        return True
        
    sid_limits[sid].append(now)
    return False



def is_login_limited(ip, limit=5, window=60):
...
async def cleanup_rate_limit_stores():
    """Runs every 5 minutes, prunes stale entries to prevent memory bloat"""
    while True:
        await asyncio.sleep(300)
        now = time.time()

        for store in [sid_limits, ip_limits, login_attempts]:
            stale_keys = [
                k for k, timestamps in store.items()
                if not any(now - t < 120 for t in timestamps)
            ]
            for k in stale_keys:
                del store[k]

        # Also prune disconnected IPs with 0 connections
        stale_ips = [ip for ip, count in ip_connection_counts.items() if count <= 0]
        for ip in stale_ips:
            del ip_connection_counts[ip]

        print(f"[CLEANUP] Rate limit stores pruned")

async def init_cleanup(app):
    asyncio.create_task(cleanup_rate_limit_stores())

#def is_rate_limited(user_id, limit=2, window=5):
...
async def hourly_counter():
        ...
        {"slot": s[0], "count": s[1]}
         ...
         await sio.emit("requests_per_day_time_acceptor",{"hours":hours_json},room="admins_pool")
         asyncio.create_task(get_daily_groom_deliveries_function())
         asyncio.create_task(get_average_today_requests_per_groom_function())
         asyncio.create_task(get_average_execution_time_per_groom_for_today_function())
         asyncio.create_task(get_average_time_to_complete_a_task_today_function())
...
async def daily_counter():
    ...
    while True:
     ...
     await sio.emit("average_requests_per_day_time_acceptor",{"hours":average_hours_json},room="admins_pool")
     asyncio.create_task(get_top_2_locations_function())
     asyncio.create_task(get_top_2_rooms_function())
     asyncio.create_task(get_daily_groom_deliveries_function())
     asyncio.create_task(get_all_time_groom_deliveries_function())
     asyncio.create_task(get_average_today_requests_per_groom_function())
     asyncio.create_task(get_seven_days_average_function())
     asyncio.create_task(get_thirty_days_average_function())
     asyncio.create_task(get_seven_days_groom_deliveries_function())
     asyncio.create_task(get_thirty_days_groom_deliveries_function())
     asyncio.create_task(get_average_execution_time_per_groom_for_today_function())
     asyncio.create_task(get_average_execution_time_per_groom_7_days_function())
     asyncio.create_task(get_average_execution_time_per_groom_30_days_function())
     asyncio.create_task(get_average_execution_time_per_groom_all_time_function())
     asyncio.create_task(get_average_time_to_complete_a_task_today_function())
     asyncio.create_task(get_average_time_to_complete_a_task_7_days_function())
     asyncio.create_task(get_average_time_to_complete_a_task_30_days_function())
     asyncio.create_task(get_average_time_to_complete_a_task_all_time_function())
...
def validate_string(value, max_len: int, field: str = "field") -> tuple[bool, str]:
...
def validate_int(value, field: str = "field") -> tuple[bool, str]:
...
async def init_db(app):
    ...
    db_pool = await asyncpg.create_pool(
        host="dpg-d75dn8kr85hc73885ho0-a.frankfurt-postgres.render.com",
        user="nostosdb",
        password="4HWrDhNmied2owPKVbScAdbxIpp3dGFw",
        database="nostosdb_tfmd",
        port=5432,
        min_size=2,
        max_size=50
    )
...
print(ph.hash("niggasoft"))
print(ph.hash("aponga"))
print(ph.hash("paraschou070806"))
print(ph.hash("pavelodisia"))
print(ph.hash("andreadisodisia"))
#print(ph.hash("1234"))
JWT_SECRET = "testing"
...
app.on_startup.append(init_cleanup)
sio.attach(app)
...
async def connect(sid, environ):
    ...
    # Block if too many connections from same IP
    if ip_connection_counts[ip] >= MAX_CONNECTIONS_PER_IP:
        print(f"[DOS] Connection rejected from {ip} — too many connections")
        return False  # returning False rejects the connection

    # Block if connecting too fast
    if is_ip_limited(ip, limit=20, window=10):
        print(f"[DOS] Connection rate limited from {ip}")
        return False

    ip_connection_counts[ip] += 1
    sid_to_ip[sid] = ip  # store mapping for disconnect cleanup
    print(f"Client connected: {sid} from {ip}")
...
async def disconnect(sid):
    ...
    if ip:
        ip_connection_counts[ip] = max(0, ip_connection_counts[ip] - 1)
    print(f"Client disconnected: {sid}")
...
async def login(sid, data):
    ...
    ip = sid_to_ip.get(sid, "unknown")
    if is_login_limited(ip):
        print(f"[DOS] Login rate limited for IP {ip}")
        await sio.emit('login_response', {"success": False, "reason": "rate_limited"}, to=sid)
        return
    if is_sid_limited(sid):
      return
    ...
    if user_id in connected_admins.values():
        ...
        async with app['db_pool'].acquire() as conn:
         sql_user = await conn.fetchrow("SELECT userid,name, active,password FROM admins WHERE userid = $1", user_id)
        ...
        name = sql_user['name']
        await sio.enter_room(sid, "admins_pool")
        #auth = data.get("auth")
        await sio.emit("Admin_Login",{"success": True,"id":user_id,"name":name,"token":token},to=sid)
        #if auth == True:
           #await sio.emit("Admin_Login",{"success": True,"id":user_id,"name":name,"token":token,"auth":True},to=sid)
        #else:
           #await sio.emit("Admin_Login",{"success": True,"id":user_id,"name":name,"token":token,"auth":False},to=sid)
        ...
        async with app['db_pool'].acquire() as conn:
         sql_user = await conn.fetchrow("SELECT userid,name, active,password_hash FROM users WHERE userid = $1", user_id)
        ...
        await sio.enter_room(sid, "users_pool")
        name = sql_user['name']
        await mode_handler(user_id,name,sid,reconnect_singal,"user",token)
        ...
        async with app['db_pool'].acquire() as conn:
         sql_groom = await conn.fetchrow("SELECT groomid,name, active,password_hash FROM grooms WHERE groomid = $1", user_id)
        ...
        name = sql_groom['name']
        await mode_handler(user_id,name,sid,reconnect_singal,"groom",token)
     ...
     async with app['db_pool'].acquire() as conn:
         ...
         sql_groom = await conn.fetchrow("SELECT groomid,name, active,password_hash FROM grooms WHERE groomid = $1", user_id)
         #print(f"[SQL_GROOM] {sql_groom}")
         sql_admin = await conn.fetchrow("SELECT adminid,name, active,password FROM admins WHERE adminid = $1", user_id)
         #print(f"[SQL_ADMIN] {sql_admin["adminid"]}")
         #print(f"[SQL_ADMIN_ID]{sql_admin}")
     ...
     if sql_admin and sql_admin['active']:#search for the user id in allowed users list // check if the username is allowed
         ...
         connected_clients[sid] = user_id
         connected_clients_via_userid[user_id] = sid
         connected_users.append({"id": user_id, "page": False, "timebool": False})
       ...
       ############################ STANDART LOGIN FUNCTIONALITY // IF THE USER/GROOM IS NOT IN THE CONNECTED LISTS ############################
        ...
        await sio.enter_room(sid, "users_pool")
        name = sql_user['name']
        await sio.emit('login_response', {"success": True, "id": user_id,"name":name, "job": "user","token":token,"mode":mode[user_id],"recon":False}, to=sid) # emit standart login message that allows the user to move forward to the app
        ...
        name = sql_groom['name']
        await sio.emit('login_response', {"success": True, "id": user_id,"name":name, "job": "groom","token":token,"mode":mode[user_id],"recon":False}, to=sid)
        #groom_accepted_requests[user_id] = []
...
async def locator(sid, data):
    ...
    if is_sid_limited(sid, limit=5, window=5):
     return
    ...
    if len(str(room_number)) > 5:
        ...
        await sio.emit("room_return",{"response":2,"priority":"Room big"},to=sid)
        return
...
""", 
    ...
    asyncio.create_task(broadcast_requests())
...
async def delivery(sid, data):
    if is_sid_limited(sid, limit=10, window=5):
      return
...
""", trimmed, groom_id, RID,trimmed2)
       ...
       #print("[Rows updated]:", nostoscur.rowcount) 
        ...
        target_sid = connected_clients_via_userid.get(overrided_groom)
...
async def Break(sid, data):
   if is_sid_limited(sid, limit=2, window=10):
      return
...
""", user_id, trimmed, None)
     ...
     return True
       ...
       await sio.emit("break_confirmation",{"confirmation":False},to=connected_clients_via_userid[user_id])
...
async def update(sid,data):
 ...
 #user_id = data.get("user_id")
 asyncio.create_task(broadcast_requests())#connected_clients_via_userid[user_id])
 asyncio.create_task(broadcast_timestamps())
 asyncio.create_task(broadcast_accepted_requests(user_id,sid))###################################################################################################
...
async def AttentionRemoval(sid, data):
    ...
    # ← use index not hardcoded [0]
    if index is None or index >= len(attentions_list):
        print(f"[ERROR] Invalid index {index}")
        return

    number = attentions_list[index]['number']  # ← was always [0]

    async with app['db_pool'].acquire() as conn:
        await conn.execute("""
            UPDATE rooms SET priority = $1 WHERE room_num = $2
        """, 0, number)

    await refresh_attentions_list()
    print(attentions_list)
    editing = data.get("editing")
    await sio.emit("AttentionList", {"list": attentions_list,"editing":editing}, room="users_pool")



@sio.event
async def send_attention_list(sid,data):
    if is_sid_limited(sid):
        return
    user_id = get_verified_user(sid, data)
    if user_id is None:
...
async def refresh_attentions_list():
    global attentions_list
    async with app['db_pool'].acquire() as conn:
        rows = await conn.fetch("""
            SELECT room_num, priority 
            FROM rooms 
            WHERE priority != 0
    ...
    for row in rows:
        result.append({
            "number": row['room_num'],
            "level": await configure_priority_reverse(row['priority'])
        })
    attentions_list = result
...
async def mode_handler(user_id,name,sid,recon,job,token):
    ...
    if(recon == True):
      match mode_handled:
        ...
        case "ChatPage":
            ...
            print(f"[!!!]{isb}")
            await sio.emit('login_response', {"success": True, "id": user_id,"name":name,"token":token, "job": job,"recon":True,"mode":mode_handled,"is_breaking":isb}, to=sid) 
...
async def broadcast_requests():
   ...
   for req in requests_list:
        # Calculate: Now - Start Time = Elapsed Seconds
        req["elapsed"] = current_ts - req["time"]
    # ---------------------------
   #print(connected_users)
      ...
      for user in connected_users:
       if user['id'] == user_id:
        ...
        break  # stop after first match
         ...
         connected_admins_via_userid.pop(user_id)
         connected_admins.pop(sid)
    print(connected_grooms)
    print(connected_users)
...
async def admin_password(sid, data):
    ...
    # 3. Check if row exists (None means no user found)
       ...
       # return
    async with app['db_pool'].acquire() as conn:
        sql_admin = await conn.fetchrow("SELECT adminid, active,password FROM admins WHERE adminid = $1", admin_id)
        print(f"[HASH LENGTH 2]: {len(sql_admin['password'])}")
        print(f"[COMPARISON 2]: {password} - {sql_admin['password']}")
        if not await check_password(password, sql_admin['password']):
    ...
    #db_password = row['password'] 
     ...
     if old_sid: # if old sid is changed mark reconnection and remove the sid for the current user in the connected client list to 
        print(f"User {admin_id} reconnected! Old SID: {old_sid}, New SID: {sid}")
        connected_clients.pop(old_sid)
        connected_clients_jobs.pop(old_sid)
...
async def room_check(sid,data):
    if is_sid_limited(sid, limit=20, window=5):
      return
    user_id = get_verified_user(sid, data)
    if user_id is None:
        print(f"[SECURITY] Unauthorized locator from {sid}")
...
sql_top_2_locations = """
select r."location" as location ,count(*) as requests
from requests r 
...
sql_top_2_rooms = """
select r."roomnumber" as room ,count(*) as requests
from requests r 
...
sql_daily_groom_deliveries = """
select r.groomid as groom,count(*) as requests
from requests r
where r.insert_time::date = current_date and r.groomid is not null
group by groom 
order by requests desc
"""

sql_all_time_groom_deliveries = """
select r.groomid as groom,count(*) as requests
from requests r
where r.insert_time::date <= current_date and r.groomid is not null
group by groom 
...
sql_average_today_requests_for_groom = """
select count(*)/count(distinct r.groomid ) as avg_requests_per_groom
from requests r
where r.insert_time::date = current_date and r.groomid is not null
"""
...
select count(*)/count(distinct r.groomid ) as avg_requests_per_groom
from requests r
where r.insert_time::date >= current_date-interval '7 days' and r.groomid is not null
"""

sql_30_days_average = """
select count(*)/count(distinct r.groomid ) as avg_requests_per_groom
from requests r
where r.insert_time::date >= current_date-interval '30 days' and r.groomid is not null
"""

sql_7_days_groom_deliveries = """
select r.groomid as groom,count(*) as requests
from requests r
where r.insert_time::date >= (current_date - interval '7 days') 
and r.completed = True
and r.groomid is not null
group by groom 
order by requests desc
"""

sql_30_days_groom_deliveries = """
select r.groomid as groom,count(*) as requests
from requests r
where r.insert_time::date >= (current_date - interval '30 days') 
and r.completed = True
and r.groomid is not null
group by groom 
order by requests desc
...
sql_average_execution_time_per_groom_for_today = """
SELECT
    g.name AS groom_name,
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
...
WHERE r.completed = true
  AND r.finish_time IS NOT NULL
  AND r.accept_time IS NOT null
  and r.insert_time::date = current_date
GROUP BY g.groomid, g.name
ORDER BY average_execution_time DESC
"""

sql_average_execution_time_per_groom_7_days = """
SELECT
    g.name AS groom_name,
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
...
WHERE r.completed = true
  AND r.finish_time IS NOT NULL
  AND r.accept_time IS NOT null
  and r.insert_time::date >= current_date - interval '7 days'
GROUP BY g.groomid, g.name
ORDER BY average_execution_time DESC
"""

sql_average_execution_time_per_groom_30_days = """
SELECT
    g.name AS groom_name,
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
...
WHERE r.completed = true
  AND r.finish_time IS NOT NULL
  AND r.accept_time IS NOT null
  and r.insert_time::date >= current_date - interval '30 days'
GROUP BY g.groomid, g.name
ORDER BY average_execution_time DESC
"""

sql_average_execution_time_per_groom_all_time = """
SELECT
    g.name AS groom_name,
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
...
sql_average_time_to_complete_a_task_today = """
SELECT
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
JOIN grooms g ON r.groomid = g.groomid
...
sql_average_time_to_complete_a_task_7_days = """
SELECT
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
JOIN grooms g ON r.groomid = g.groomid
...
sql_average_time_to_complete_a_task_30_days = """
SELECT
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
JOIN grooms g ON r.groomid = g.groomid
...
sql_average_time_to_complete_a_task_all_time = """
SELECT
   to_char(AVG(r.finish_time - r.accept_time), 'HH24:MI:SS') AS average_execution_time
FROM requests r
JOIN grooms g ON r.groomid = g.groomid
...
@sio.event 
async def requests_per_day_time(sid,data):
    if is_sid_limited(sid, limit=10, window=5):
      return
    user_id = get_verified_user(sid, data)
    if user_id is None:
...
@sio.event
async def average_groom_completion_time(sid,data):
    if is_sid_limited(sid, limit=10, window=5):
      return
    user_id = get_verified_user(sid, data)
    if user_id is None:
...
@sio.event
async def admin_total_daily_requests_sender_event(sid):
     global total_daily_requests
...
async def admin_total_daily_requests_sender():
    ...
    await sio.emit("total_daily_requests_acceptor",{"total_daily_requests":total_daily_requests},room="admins_pool")
...
async def late_requests_for_today_sender():
    ...
    await sio.emit("late_requests_for_today_acceptor",{"late_requests_for_today":row},room="admins_pool")
...
@sio.event
async def top_2_locations(sid):
     async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_top_2_locations, 2)
          print(f"[ROW]: {row}")
          top_2_locations = [
    {"location": s[0], "requests": s[1]}
              for s in row
]
     print(f"[LOCATIONS]")
     await sio.emit("top_2_locations",{"top_2_locations":top_2_locations},to=sid)
     print(f"[EMITTED]")

@sio.event
async def top_2_rooms(sid):
     async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_top_2_rooms, 2)
          print(f"[ROW]: {row}")
          top_2_rooms = [
    {"room": s[0], "requests": s[1]}
              for s in row
]
     print(f"[ROOMS]")
     await sio.emit("top_2_rooms",{"top_2_rooms":top_2_rooms},to=sid)
     print(f"[EMITTED]")

@sio.event
async def daily_groom_deliveries(sid):
     async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_daily_groom_deliveries)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "deliveries": s[1]}
              for s in row
]
     await sio.emit("daily_groom_deliveries",{"daily_groom_deliveries":deliveries},to=sid)
     print(f"[EMITTED]")

@sio.event
async def all_time_groom_deliveries(sid):
     async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_all_time_groom_deliveries)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "deliveries": s[1]}
              for s in row
]
     await sio.emit("all_time_groom_deliveries",{"all_time_groom_deliveries":deliveries},to=sid)
     print(f"[EMITTED]")


@sio.event 
async def average_today_requests_per_groom(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetchval(sql_average_today_requests_for_groom)
          print(f"[ROW]: {row}")
    await sio.emit("average_today_requests_per_groom",{"average_today_requests_per_groom":row},to=sid)
    print(f"[EMITTED]")

@sio.event
async def seven_days_average(sid):
     async with app['db_pool'].acquire() as conn:
          row = await conn.fetchval(sql_7_days_krousan)
          print(f"[ROW]: {row}")
     await sio.emit("seven_days_average",{"seven_days_average":row},to=sid)
     print(f"[EMITTED]")

@sio.event 
async def thirty_days_average(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetchval(sql_30_days_average)
          print(f"[ROW]: {row}")
    await sio.emit("thirty_days_average",{"thirty_days_average":row},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def seven_days_groom_deliveries(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_7_days_groom_deliveries)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "deliveries": s[1]}
              for s in row
]
    await sio.emit("seven_days_groom_deliveries",{"seven_days_groom_deliveries":deliveries},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def thirty_days_groom_deliveries(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_30_days_groom_deliveries)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "deliveries": s[1]}
              for s in row
]
    await sio.emit("thirty_days_groom_deliveries",{"thirty_days_groom_deliveries":deliveries},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_execution_time_per_groom_for_today(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_execution_time_per_groom_for_today)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "time": s[1]}
              for s in row
]
    await sio.emit("average_execution_time_per_groom_for_today",{"average_execution_time_per_groom_for_today":deliveries},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_execution_time_per_groom_7_days(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_execution_time_per_groom_7_days)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "time": s[1]}
              for s in row
]
    await sio.emit("average_execution_time_per_groom_7_days",{"average_execution_time_per_groom_7_days":deliveries},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_execution_time_per_groom_30_days(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_execution_time_per_groom_30_days)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "time": s[1]}
              for s in row
]
    await sio.emit("average_execution_time_per_groom_30_days",{"average_execution_time_per_groom_30_days":deliveries},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_execution_time_per_groom_all_time(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_execution_time_per_groom_all_time)
          print(f"[ROW]: {row}")
          deliveries = [
    {"groom": s[0], "time": s[1]}
              for s in row
]
    await sio.emit("average_execution_time_per_groom_all_time",{"average_execution_time_per_groom_all_time":deliveries},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_time_to_complete_a_task_today(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_time_to_complete_a_task_today)
          print(f"[ROW]: {row}")
          times = [
    {"execution_time": s[0]}
              for s in row
]
    await sio.emit("average_time_to_complete_a_task_today",{"average_time_to_complete_a_task_today":times},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_time_to_complete_a_task_7_days(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_time_to_complete_a_task_7_days)
          print(f"[ROW]: {row}")
          times = [
    {"execution_time": s[0]}
              for s in row
]
    await sio.emit("average_time_to_complete_a_task_7_days",{"average_time_to_complete_a_task_7_days":times},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_time_to_complete_a_task_30_days(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_time_to_complete_a_task_30_days)
          print(f"[ROW]: {row}")
          times = [
    {"execution_time": s[0]}
              for s in row
]
    await sio.emit("average_time_to_complete_a_task_30_days",{"average_time_to_complete_a_task_30_days":times},to=sid)
    print(f"[EMITTED]")

@sio.event 
async def average_time_to_complete_a_task_all_time(sid):
    async with app['db_pool'].acquire() as conn:
          row = await conn.fetch(sql_average_time_to_complete_a_task_all_time)
          print(f"[ROW]: {row}")
          times = [
    {"execution_time": s[0]}
              for s in row
]
    await sio.emit("average_time_to_complete_a_task_all_time",{"average_time_to_complete_a_task_all_time":times},to=sid)
    print(f"[EMITTED]")

async def get_top_2_locations_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_top_2_locations, 2)
        print(f"[ROW]: {row}")
        top_2_locations = [{"location": s[0], "requests": s[1]} for s in row]
    print(f"[LOCATIONS]")
    await sio.emit("top_2_locations", {"top_2_locations": top_2_locations}, room="admins_pool")


async def get_top_2_rooms_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_top_2_rooms, 2)
        print(f"[ROW]: {row}")
        top_2_rooms = [{"room": s[0], "requests": s[1]} for s in row]
    print(f"[ROOMS]")
    await sio.emit("top_2_rooms", {"top_2_rooms": top_2_rooms}, room="admins_pool")


async def get_daily_groom_deliveries_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_daily_groom_deliveries)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "deliveries": s[1]} for s in row]
    await sio.emit("daily_groom_deliveries", {"daily_groom_deliveries": deliveries}, room="admins_pool")


async def get_all_time_groom_deliveries_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_all_time_groom_deliveries)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "deliveries": s[1]} for s in row]
    await sio.emit("all_time_groom_deliveries", {"all_time_groom_deliveries": deliveries}, room="admins_pool")


async def get_average_today_requests_per_groom_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetchval(sql_average_today_requests_for_groom)
        print(f"[ROW]: {row}")
    await sio.emit("average_today_requests_per_groom", {"average_today_requests_per_groom": row}, room="admins_pool")


async def get_seven_days_average_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetchval(sql_7_days_krousan)
        print(f"[ROW]: {row}")
    await sio.emit("seven_days_average", {"seven_days_average": row}, room="admins_pool")


async def get_thirty_days_average_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetchval(sql_30_days_average)
        print(f"[ROW]: {row}")
    await sio.emit("thirty_days_average", {"thirty_days_average": row}, room="admins_pool")


async def get_seven_days_groom_deliveries_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_7_days_groom_deliveries)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "deliveries": s[1]} for s in row]
    await sio.emit("seven_days_groom_deliveries", {"seven_days_groom_deliveries": deliveries}, room="admins_pool")


async def get_thirty_days_groom_deliveries_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_30_days_groom_deliveries)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "deliveries": s[1]} for s in row]
    await sio.emit("thirty_days_groom_deliveries", {"thirty_days_groom_deliveries": deliveries}, room="admins_pool")


async def get_average_execution_time_per_groom_for_today_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_execution_time_per_groom_for_today)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "time": s[1]} for s in row]
    await sio.emit("average_execution_time_per_groom_for_today", {"average_execution_time_per_groom_for_today": deliveries}, room="admins_pool")


async def get_average_execution_time_per_groom_7_days_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_execution_time_per_groom_7_days)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "time": s[1]} for s in row]
    await sio.emit("average_execution_time_per_groom_7_days", {"average_execution_time_per_groom_7_days": deliveries}, room="admins_pool")


async def get_average_execution_time_per_groom_30_days_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_execution_time_per_groom_30_days)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "time": s[1]} for s in row]
    await sio.emit("average_execution_time_per_groom_30_days", {"average_execution_time_per_groom_30_days": deliveries}, room="admins_pool")


async def get_average_execution_time_per_groom_all_time_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_execution_time_per_groom_all_time)
        print(f"[ROW]: {row}")
        deliveries = [{"groom": s[0], "time": s[1]} for s in row]
    await sio.emit("average_execution_time_per_groom_all_time", {"average_execution_time_per_groom_all_time": deliveries}, room="admins_pool")


async def get_average_time_to_complete_a_task_today_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_time_to_complete_a_task_today)
        print(f"[ROW]: {row}")
        times = [{"execution_time": s[0]} for s in row]
    await sio.emit("average_time_to_complete_a_task_today", {"average_time_to_complete_a_task_today": times}, room="admins_pool")


async def get_average_time_to_complete_a_task_7_days_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_time_to_complete_a_task_7_days)
        print(f"[ROW]: {row}")
        times = [{"execution_time": s[0]} for s in row]
    await sio.emit("average_time_to_complete_a_task_7_days", {"average_time_to_complete_a_task_7_days": times}, room="admins_pool")


async def get_average_time_to_complete_a_task_30_days_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_time_to_complete_a_task_30_days)
        print(f"[ROW]: {row}")
        times = [{"execution_time": s[0]} for s in row]
    await sio.emit("average_time_to_complete_a_task_30_days", {"average_time_to_complete_a_task_30_days": times}, room="admins_pool")


async def get_average_time_to_complete_a_task_all_time_function():
    async with app['db_pool'].acquire() as conn:
        row = await conn.fetch(sql_average_time_to_complete_a_task_all_time)
        print(f"[ROW]: {row}")
        times = [{"execution_time": s[0]} for s in row]
    await sio.emit("average_time_to_complete_a_task_all_time", {"average_time_to_complete_a_task_all_time": times}, room="admins_pool")
