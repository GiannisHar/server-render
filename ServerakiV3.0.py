...
WHERE name IS NOT NULL
  AND fullfilled IS FALSE
  AND canceled IS FALSE
  AND message_deleted IS FALSE
ORDER BY id DESC
LIMIT %s OFFSET %s"""
...
async def send_message(sid,data):
    ...
    try:
        values = (RID, user_id, message,now)
...
async def admin_log(sid,data):
    user_id = data.get("user_id")
    old_sid = None
    for sid_,uid in connected_admins:
        if uid == user_id:
         sid_ = sid
