...
def login(sid, data):
    ...
    if any(use['id'] == user_id for use in connected_users): #search again for the user id only this time in the user list
        ...
        sio.emit('login_response', {"success": True, "id": user_id, "job": "user","mode":mode[user_id],"recon":True}, to=sid) #emit standart login message that allows the user to move forward to the app
        ...
        sio.emit('login_response', {"success": True, "id": user_id, "job": "groom","mode":mode[user_id],"recon":True}, to=sid)
