...
valid_rooms = [1001,1002,1003,1004,1005]
...
def disconnect(sid):
    ...
    job = connected_clients_jobs[sid]
    print(f"[JOBING OF JOBINGTONS] {job}")
    eventlet.sleep(1)

    was_paused = user_id in paused_app
    was_detached = user_id in detached_app

    print(f"is it paused? {was_paused}")
    print(f"is it detached {was_detached}")
    if detached_app:
     if(job == "groom"):
      ...
      print(f"[/!\\] (GROOM) user: {user_id} got away")
     
     else:
          print(f"[/!\\] user (USER): {user_id} got away")
    
    paused_app.discard(user_id)
    detached_app.discard(user_id)"""
...
def app_paused(sid,data):
    ...
    #paused_app.add(user_id)
    #print("[PAUSE MY FRIEND]")



@sio.event 
def app_closed(sid,data):
    user_id = data.get("user_id")
    ...
    """if(job == "groom"):
     ...
     accepted_rids.difference_update(rid_list)
     groom_accepted_requests[user_id] = []
     print(f"[/!\\] user: {user_id} got away")
    else:
         print(f"[/!\\] user: {user_id} got away")"""


@sio.event 
def battery(sid,data):
   user_id = data.get("user_id")
   ...
   if battery <= 5 and charging != "BatteryState.charging" and user_id not in wakelock_mode:
    sio.emit("Low_Battery",{"act":True},to=sid)
    ...
    sio.emit("Low_Battery",{"act":False},to=sid)
...
def login(sid, data):
    ...
    if any(use['id'] == user_id for use in connected_users): #search again for the user id only this time in the user list
     ...
     if any(user['id'] == user_id and user['status'] == 'active'for user in users): #search for the user id in allowed users list // check if the username is allowed
        ...
        ############################ EXACT SAME THING FOR GROOMS AND CONNECTED_GROOM LIST ############################
     elif any(groom['id'] == user_id and groom['status'] == 'active' for groom in grooms) or (groom['id'] == 'pmurans' and groom['status'] == 'active' for groom in grooms) or (groom['id'] == 'kandreadis' and groom['status'] == 'active' for groom in grooms):
...
def locator(sid, data):
    ...
    eventlet.spawn(broadcast_user_active_requests,user_id,sid)
    eventlet.spawn(broadcast_requests,sid)
    eventlet.spawn(broadcast_timestamps)
    print(requests_list) #debug message 
    print(connected_users) #debug message
####################################################################################
####################################################################################
@sio.event 
def get_accepted_requests(sid,data):
    user_id = data.get("user_id")
    mode = data.get("mode")
    eventlet.spawn(broadcast_accepted_requests,user_id,sid)

####################################################################################

####################################################################################
@sio.event 
def get_user_active_requests(sid,data):
    user_id = data.get("user_id")
    mode = data.get("mode")
    eventlet.spawn(broadcast_user_active_requests,user_id,sid)
####################################################################################
...
def delete_request(sid,data):
    ...
    # broadcast updates
    eventlet.spawn(broadcast_requests, sid)
    eventlet.spawn(broadcast_accepted_requests, user_id, -1)
    eventlet.spawn(broadcast_user_active_requests, user_id, sid)
...
def delivery(sid, data):
    ...
    if(action == 0):
       ...
       if RID in accepted_rids:
        # Already taken
        sio.emit("call_taken", {"taken_rid": RID}, to=sid)
        return 
       ...
       sio.emit("permition_granded", {"taken_rid": RID}, to=sid)
       ...
       print(accepted_rids)
        ...
        overrider[RID] = groom_id
        eventlet.spawn(broadcast_accepted_requests,overrided_groom,target_sid)
        
    eventlet.spawn(broadcast_requests,sid)
    eventlet.spawn(broadcast_accepted_requests,groom_id,sid)
    eventlet.spawn(broadcast_timestamps)  
...
def Arrived(sid, data):
    ...
    eventlet.spawn(broadcast_requests,sid)
    eventlet.spawn(broadcast_accepted_requests,user_id,sid)
    eventlet.spawn(broadcast_user_active_requests,user_id,target_sid)
...
def update(sid,data):
 ...
 user_id = data.get("user_id")
 eventlet.spawn(broadcast_requests,1)#connected_clients_via_userid[user_id])
 eventlet.spawn(broadcast_timestamps)
...
@sio.event 
def personal_requests(sid,data):
    user_id = data.get("user_id")
...
def broadcast_timestamps():
    for i in range(5):
        for rid, ts in request_timestamps.items():
            sio.emit("timestamp", {"RID": rid, "timestamp": ts})
        eventlet.sleep(1)  # emit every second
...
def broadcast_accepted_requests(groom_id,sid):
   print("get ready")
   print(sid)
   #print(connected_users)
   #print(connected_grooms)
   ...
   if sid != -1:
    for i in range(5):
      #sio.emit("Board", requests_list,to=sid)
      sio.emit("accepted_requests",groom_accepted_requests[groom_id],to=sid)
      eventlet.sleep(1)
   else:
      for target_sid, job in connected_clients_jobs.items():
         ...
         if groom_id not in groom_accepted_requests:
            groom_accepted_requests[groom_id] = []
         sio.emit("accepted_requests", groom_accepted_requests[groom_id],to=target_sid)
...
def broadcast_user_active_requests(user_id,sid):
   print("get ready")
   print(sid)
   #print(connected_users)
   #print(connected_grooms)
   ...
   for i in range(5):
     #sio.emit("Board", requests_list,to=sid)
     sio.emit("user_active_requests",user_active_requests[user_id],to=sid)
     eventlet.sleep(1)
...
def SortRequestsList(req_list):
    ...
    def priority_key(req):
     ...
     time_bucket = req.get("time", 0) // 120
